 ...
 */
static char *font = "Liberation Mono:pixelsize=14:style=Bold:antialias=true:autohint=true";
static char *font2[] = {
	"Liberation Mono:pixelsize=14:style=Bold:antialias=true:autohint=true"
};
...
#define TERMMOD (Mod4Mask|MODKEY)
...
static char *copyurlcmd[] = { "/bin/sh", "-c",
  "tmp=$(sed 's/.*│//g' | tr -d '\n' | grep -aEo '(((http|https|gopher|gemini|ftp|ftps|git)://|www\\.)[a-zA-Z0-9.]*[:]?[a-zA-Z0-9./@$&%?$#=_-~]*)|((magnet:\\?xt=urn:btih:)[a-zA-Z0-9]*)' | \
        uniq | sed 's/^www./http:\\/\\/www\\./g' ); IFS=; [ ! -z $tmp ] && \
        echo $tmp | dmenu -i -p 'Copy which url?' -l 10 | tr -d '\n' | xclip -selection clipboard",
  "externalpipe", NULL };
